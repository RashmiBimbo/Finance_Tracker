    ...
    public partial class Performance : Page
    {
        ...
        private DataTable GVReports3DS
        {
            get
            {
                ...
                if (!(dt?.Rows.Count > 0))
                {
                    ...
                    string User_Id = Session["User_Id"]?.ToString().Trim().ToUpper();
        ...
        private string ConstructJSON_M(out int chkCnt)
        {
            ...
            foreach (GridViewRow gvRow in GVAdd.Rows)
            {
                ...
                {
                    {"USER_ID", Session["User_Id"]?.ToString().Trim().ToUpper()},
                    ...
                    {"LOCATION", fullPath},
                    {"CREATED_BY", Session["User_Name"] ?.ToString().Trim()}
                };
                dtls.Add(paramVals);
                cb.Checked = false;
                chKCount--;
        ...
        private void ResetTab1()
        {
            ...
            string roleId = Session["Role_Id"]?.ToString().Trim();
